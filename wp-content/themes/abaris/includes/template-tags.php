...
/*
 ...
 if( ! function_exists( 'abaris_breadcrumbs' )) {
	...
	function abaris_breadcrumbs() {
		global $abaris;
		/* === OPTIONS === */
		$text['home']     = '<i class="el el-home"></i>'; // text for the 'Home' link
...
function abaris_posted_on() {
	...
	$byline = sprintf(
		...
		'<span class="author vcard"><i class="el el-user"></i> <a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
	);

	echo '<span class="posted-on"><i class="el el-time"></i> ' . $posted_on . '</span><span class="byline"> ' . $byline . '</span>';

	if ( ! post_password_required() && ( comments_open() ) ) : ?>
		<span class="comments-link"><i class="el el-comment-alt"></i> <?php comments_popup_link( __( 'Leave a comment', 'abaris' ), __( '1 Comment', 'abaris' ), __( '% Comments', 'abaris' ) ); ?></span>
	<?php
	endif;
	?>
	...
	<?php
		if (! is_single() ) { 
			printf( __('<span class="read-more-link"><i class="el el-link"></i> %1$s</span>', 'abaris' ),
...
function abaris_post_date() {
	...
	printf( __( '<span class="posted-on"><i class="el el-time"></i> %1$s</span>', 'abaris' ),
		sprintf( '<a href="%1$s" rel="bookmark">%2$s</a>',
			esc_url( get_permalink() ),
			$time_string
		)
...
function the_archive_title( $before = '', $after = '' ) {
	if ( is_category() ) {
		$title = sprintf( __( 'Category: %s', 'abaris' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		$title = sprintf( __( 'Tag: %s', 'abaris' ), single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		$title = sprintf( __( 'Author: %s', 'abaris' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		$title = sprintf( __( 'Year: %s', 'abaris' ), get_the_date( _x( 'Y', 'yearly archives date format', 'abaris' ) ) );
	} elseif ( is_month() ) {
		$title = sprintf( __( 'Month: %s', 'abaris' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'abaris' ) ) );
	} elseif ( is_day() ) {
		$title = sprintf( __( 'Day: %s', 'abaris' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'abaris' ) ) );
	} elseif ( is_tax( 'post_format', 'post-format-aside' ) ) {
		$title = _x( 'Asides', 'post format archive title', 'abaris' );
	} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
		$title = _x( 'Galleries', 'post format archive title', 'abaris' );
	} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
		$title = _x( 'Images', 'post format archive title', 'abaris' );
	} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
		$title = _x( 'Videos', 'post format archive title', 'abaris' );
	} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
		$title = _x( 'Quotes', 'post format archive title', 'abaris' );
	} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
		$title = _x( 'Links', 'post format archive title', 'abaris' );
	} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
		$title = _x( 'Statuses', 'post format archive title', 'abaris' );
	} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
		$title = _x( 'Audio', 'post format archive title', 'abaris' );
	} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
		$title = _x( 'Chats', 'post format archive title', 'abaris' );
	} elseif ( is_post_type_archive() ) {
		$title = sprintf( __( 'Archives: %s', 'abaris' ), post_type_archive_title( '', false ) );
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
		$title = sprintf( __( '%1$s: %2$s', 'abaris' ), $tax->labels->singular_name, single_term_title( '', false ) );
	} else {
		$title = __( 'Archives', 'abaris' );
	}
